...
const relatedproduct = (props: any) => {
    ...
    return (
        <div className="relate_main mt-[80px]">
            <h2 className='relative font-bold text-center border-primary text-black mb-8 md:text-2xl mx-auto block pb-2 w-max before:cursor-context-menu before:left-1/2 before:bottom-0 before:w-1/2 before:h-[1px] before:bg-secondary before:absolute before:block before:translate-x-[-50%] '>You may also like</h2>
            <div className="prod-with-detail prod-detila flex md:flex-nowrap flex-wrap justify-center gap-8">
                
                    {
                        relateddata.map((data: any) => {
                            return (
                                <div className="text-center product-main w-full md:max-w-[45%]  md:w-[100%]  bg-white shadow-md rounded-xl duration-500 border border-[#0005]" key={data.id}>

...
const CollectionList = () =>{
  ...
  const homeCollection = async ()=>{
    ...
    return(
        ...
        <section className="container md:pt-[40px] pt-[25px] flex justify-between flex-wrap">            
            {
                    ...
                    return(                    
                        ...
                        <div key={data.id+1} className="main relative md:min-w-[23%] md:max-w-[23%] max-w-full mb-7 md:mb-0">
                            ...
                            <div className="info absolute bottom-[30px] px-12 w-full text-center">                                
                                <Link href={`/categories/${data.slug}`} className="bg-white py-3 px-4 md:text-[16px] text-[14px] font-bold  rounded-md mx-auto inline-block border-2 border-[#251048]">{data.name}</Link>

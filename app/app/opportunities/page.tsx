...
const opportunities = () => {
    ...
    console.log("fingal h h", opportunity);
    ...
    const opportunityData = async () => {
        try{
            ...
            const finalResult = await result.careers;
            //console.log('result hai', result.careers);
            setOopportunity(finalResult);
                            ...
                            <table className="w-full text-sm text-left rtl:text-right text-white-100 dark:text-white">
                                <thead className="text-xs uppercase bg-primary dark:text-white text-white">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3 whitespace-nowrap">Position</th>
                                ...
                                <tbody>
                                            {opportunity.map((data:any, index:number) =>{
                                                return(
                                                    <>
                                                    <tr key={index++}>
                                                    <td className="px-6 py-4 border-b dark:bg-gray-800 dark:border-gray-700">{index+1}</td>
                                                    <td className="px-6 py-4 border-b dark:bg-gray-800 dark:border-gray-700">{data.name}</td>
                                                    <td className="px-6 py-4 border-b dark:bg-gray-800 dark:border-gray-700">{data.deartment}</td>
                                                    <td className="px-6 py-4 border-b dark:bg-gray-800 dark:border-gray-700">{data.location}</td>
                                                    <td className="px-6 py-4 border-b dark:bg-gray-800 dark:border-gray-700"><Link href={`/opportunities/careers/${data.career_id}`} className="font-bold border-b border-primary">Apply Now</Link></td>
                                    </tr>
                                                    </>
                                                )
                                            })
                ...
                <div className={activetab == 2 ? 'block' : 'hidden'} data-attr="2">
                    ...
                    <table className="w-full text-sm text-left rtl:text-right text-white-100 dark:text-white">
                                ...
                                <tr>
                                                    ...
                                                    <ul className="flex flex-wrap justify-center md:my-2 ">

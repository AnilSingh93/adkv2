        ...
        <section className="container md:pt-[40px] pt-[40px] flex justify-between flex-wrap">            
            {
                  collectionList.slice(Totallength,tillnum).map((data:any)=>{
                    return(                    
                        ...
                        <div key={data.id+1} className="main relative md:min-w-[24.5%] md:max-w-[24.5%] max-w-full mb-7 md:mb-0">
                            <div className="relative after:bg-gradient-to-t from-[#27367b30] to-[#4017833d] after:content:'' after:absolute after:w-full after:h-full after:left-0 after:top-0  md:h-[300px] h-[auto] main_home_coll">
                            ...
                            <div className="info absolute bottom-[30px] px-12 w-full text-center">                                
                                <Link href={`/category/${data.slug}`} className="bg-white py-3 px-4 md:text-[16px] text-[14px] font-bold  rounded-md mx-auto inline-block">{data.name}</Link>
